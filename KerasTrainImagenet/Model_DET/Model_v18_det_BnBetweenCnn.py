...
def prepModel( input_shape=(150,150,3), \
    ...
    base_model.add( Convolution2D ( filters = L2_filters,  kernel_size = (L2_size, L2_size),  strides = (L2_stride, L2_stride), padding = Conv_padding ) )
    ...
    base_model.add( Convolution2D ( filters = L4_filters,  kernel_size = (L4_size, L4_size),  strides = (L4_stride, L4_stride), padding = Conv_padding ) )
    base_model.add(BatchNormalization(center=False, scale=False))
    base_model.add(Activation('relu'))
    if L4_dropout > 0.:
