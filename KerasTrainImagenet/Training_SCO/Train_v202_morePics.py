...
# model file
model_file_pattern = "A:\\RetellectModels\\model_{}_{}prekes.h5"
lc_file_pattern = "A:\\RetellectModels\\lc_{}_{}prekes.csv"
...
def trainModel( epochs = 1):
    ...
    lc_file_name = lc_file_pattern.format( date.today().strftime("%Y%m%d"), Softmax_size )
    callback_csv_logger = CSVLogger(lc_file_name + '.csv', separator=",", append=False)

    #callback_earlystop = EarlyStopping ( monitor='val_acc', min_delta=0., patience=0, verbose=2, mode='auto', restore_best_weights=True )
    model_file_name = model_file_pattern.format( date.today().strftime("%Y%m%d"), Softmax_size )
    ...
    model.fit_generator ( dataGen, steps_per_epoch=len(dataGen), epochs=epochs, verbose=2, 
                         validation_data=vldDataGen, validation_steps=len(vldDataGen), callbacks=[callback_earlystop,mcp_save,callback_csv_logger] )
    #model.fit_generator ( dataGen, steps_per_epoch=len(dataGen), epochs=epochs, verbose=2 )
