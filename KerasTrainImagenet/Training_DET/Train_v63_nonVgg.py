...
#
# To run:
#   model = t_det_v63.trainModel( epochs=30, stepsPerEpochFull=False, stepsPerEpoch=5)
#   model = t_det_v63.trainModel(model, epochs=50, stepsPerEpochFull=False, stepsPerEpoch=5)
#   model = t_det_v63.trainModel(model, epochs=10)
...
def trainModel( model = None, epochs = 1, batch_size=32, stepsPerEpochFull = True, stepsPerEpoch = 0):
    ...
    if model is None:
        model = m_det_v14.prepModel ( input_shape=(target_size,target_size,3) )
